...
export function formatMetricDataByCategory(data = {}) {
  ...
  const categorySet = items.map((item) => item.type)
  ...
  const res = categorySetNoRepeat.map((category) => {
    ...
    const filterCate = items.filter((item) => item.type === category)
    ...
    const formatFilterCate = filterCate.map((plugin_item) => {
      ...
      // })
      // return { ...plugin_item, detail: filterDetail }

      const { project_distribution = [] } = plugin_item
      const filterDetail = project_distribution.map((d) => {
        const formatRate = d.rate && (d.rate * 100).toFixed(2) + '%'
        return {
          ...d,
          rate: formatRate,
      ...
      return { ...plugin_item, detail: filterDetail, plugin_code: plugin_item.name }
    })
